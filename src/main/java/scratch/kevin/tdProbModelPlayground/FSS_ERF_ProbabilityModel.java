 ...
 */
public interface FSS_ERF_ProbabilityModel {
	 ...
	 */
	public static class Poisson implements FSS_ERF_ProbabilityModel {

		@Override
		public double getProbability(FaultSystemSolution fss, int fltSysRupIndex,
				long forecastStartTimeMillis, double durationYears) {
			return 1d - Math.exp(-fss.getRateForRup(fltSysRupIndex) * durationYears);
		}
	 ...
	 */
	public static class WeightedCombination implements FSS_ERF_ProbabilityModel {
		
		private WeightedList<? extends FSS_ERF_ProbabilityModel> probModels;

		public WeightedCombination(WeightedList<? extends FSS_ERF_ProbabilityModel> probModels) {
		...
		@Override
		public double getProbability(FaultSystemSolution fss, int fltSysRupIndex, long forecastStartTimeMillis,
				double durationYears) {
			double ret = 0d;
			for (WeightedValue<? extends FSS_ERF_ProbabilityModel> value : probModels)
				ret += value.weight * value.value.getProbability(fss, fltSysRupIndex, forecastStartTimeMillis, durationYears);
		...
		public double getProbabilityGain(FaultSystemSolution fss, int fltSysRupIndex, long forecastStartTimeMillis,
			...
			double ret = 0d;
			for (WeightedValue<? extends FSS_ERF_ProbabilityModel> value : probModels)
				ret += value.weight * value.value.getProbabilityGain(fss, fltSysRupIndex, forecastStartTimeMillis, durationYears);
			// already normalized in the constructor, don't need to re-normalize
			return ret;
		}
