...
//		CPT cpt = new CPT(cptMin, cptMax, new Color(0, 0, 255, 40), new Color(255, 0, 0, 40));
		...
		for (boolean subOnly : new boolean[] {false,true}) {
			File outputDir;
			double cptMin, cptMax;
			double[] minMags;
			TectonicRegionType[] trts;
			if (subOnly) {
				outputDir = new File(FIGURES_DIR, "sub_partic_rate_maps");
				cptMin = -5d;
				cptMax = -3d;
				minMags = new double[] { 0d, 7d, 7.5d, 8d, 8.5, 9d };
				// now gridded
				trts = new TectonicRegionType[] {
						TectonicRegionType.SUBDUCTION_INTERFACE,
						TectonicRegionType.SUBDUCTION_SLAB
				};
			} else { 
				outputDir = new File(FIGURES_DIR, "combined_partic_rate_maps");
				cptMin = -6d;
				cptMax = -2d;
				minMags = new double[] { 0d, 6d, 6.5, 7d, 7.5d, 8d };
				// now gridded
				trts = new TectonicRegionType[] {
						null,
