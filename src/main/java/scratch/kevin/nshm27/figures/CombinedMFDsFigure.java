...
//		double[] csvMags = { 6d, 7d, 7.5d, 8d, 8.5d, 9d };
		...
		for (NSHM27_SeismicityRegions seisReg : NSHM27_SeismicityRegions.values()) {
			...
			for (boolean mapRegion : new boolean[] {false,true}) {
				Region reg;
				if (mapRegion)
					reg = NSHM27_MapRegions.valueOf(seisReg.name()).load();
				else
					reg = seisReg.load();
				
				IncrementalMagFreqDist fullObsMean = null;
				EvenlyDiscretizedFunc fullObsMeanCml = null;
				...
				for (boolean interfaceOnly : new boolean[] {false, true}) {
					...
					for (TectonicRegionType trt : TRTs) {
						...
						IncrementalMagFreqDist gridMFD = loadGridded(gridList, trt, refMFD, reg);
					...
					double pdfInRegion = 1;
					if (mapRegion) {
						double totRate = 0d;
						double totRateInRegion = 0d;
						...
						for (TectonicRegionType trt : TRTs) {
							...
							PureGR rateModel = (PureGR)NSHM27_SeisRateModelBranch.PREFFERRED.getRateRecord(
									seisReg, NSHM27_SeisClassificationMethod.AVERAGE, trt);
							if (m1 < 0)
								m1 = rateModel.M1;
							else
								Preconditions.checkState(m1 == rateModel.M1);
							
							GriddedGeoDataSet pdf;
							if (trt == TectonicRegionType.SUBDUCTION_SLAB)
								pdf = NSHM27_SeisPDF_Loader.load3D(seisReg, NSHM27_SeisClassificationMethod.AVERAGE,
										trt, NSHM27_DeclusteringAlgorithms.AVERAGE, NSHM27_SeisSmoothingAlgorithms.AVERAGE).sum2D();
							else
								pdf = NSHM27_SeisPDF_Loader.load2D(seisReg, NSHM27_SeisClassificationMethod.AVERAGE,
										trt, NSHM27_DeclusteringAlgorithms.AVERAGE, NSHM27_SeisSmoothingAlgorithms.AVERAGE);
							
							double fractInRegion = 0d;
							for (int i=0; i<pdf.size(); i++)
								if (reg.contains(pdf.getLocation(i)))
									fractInRegion += pdf.get(i);
							
							totRate += rateModel.rateAboveM1;
							totRateInRegion += rateModel.rateAboveM1 * fractInRegion; 
						}
						
						pdfInRegion = totRateInRegion / totRate;
						System.out.println("PDF fract in region: "+(float)totRateInRegion+" / "+(float)totRate+" = "+(float)pdfInRegion);
					}
					...
					for (int s=0; s<samples.size(); s++) {
						...
						gr.scaleToCumRate(refMFD.getClosestXIndex(sample.M1+0.001), sample.rateAboveM1*pdfInRegion);
					...
					if (!interfaceOnly) {
						fullObsMean = obsIncrMean;
						fullObsMeanCml = obsCmlMean;
					}
...
//					EvenlyDiscretizedFunc obsCmlMean = obsIncrMean.getCumRateDistWithOffset();
					...
					if (mapRegion) {
						obsIncrMean.setName("Observed mean (scaled to region) (b="+twoDF.format(obsB)+")");
					} else {
...
//								seisReg, NSHM27_SeisClassificationMethod.PROSLAB, targetTRT, refMFD, refMFD.getMaxX());
						...
						fullObsMean.setName("Observed total mean");
						incrFuncs.add(fullObsMean);
						fullObsMeanCml.setName(fullObsMean.getName());
						cmlFuncs.add(fullObsMeanCml);
						chars.add(new PlotCurveCharacterstics(PlotLineType.SOLID, 1.5f, Colors.tab_lightbrown));
					}
...
//					PlotUtils.writePrintPlots(outputDir, prefix+"_cml_half", gp, halfWidth, halfHeight, 150, true, true, false);
	...
	private static IncrementalMagFreqDist loadGridded(GridSourceList gridList,
		...
		SummedMagFreqDist mfdSum = new SummedMagFreqDist(refMFD.getMinX(), refMFD.getMaxX(), refMFD.size());
		boolean regCheck = reg != null && !gridList.getGriddedRegion().equalsRegion(reg);
		for (int l=0; l<gridList.getNumLocations(); l++) {
			if (regCheck && !reg.contains(gridList.getLocation(l)))
				continue;
