...
public class ScenarioStiffnessCalc {
	...
	public static void main(String[] args) throws IOException {
		...
		boolean doRakeChange = false;
		double newRake = 0;
		int[] newRakeParents = {
				FaultSectionUtils.findParentSectionID(sects, "Hollywood"),
				FaultSectionUtils.findParentSectionID(sects, "Raymond")
		};
		List<FaultSection> matchingSects = new ArrayList<>();
		for (FaultSection sect : sects) {
			for (int parentID : newRakeParents) {
				if (sect.getParentSectionId() == parentID) {
					if (doRakeChange) {
						System.out.println("Changing rake on "+sect.getSectionName()+" from "+(float)sect.getAveRake()+" to "+(float)newRake);
						sect.setAveRake(newRake);
					}
		...
		SubSectStiffnessCalculator stiffnessCalc = new SubSectStiffnessCalculator(
				sects, stiffGridSpacing, 3e4, 3e4, coeffOfFriction, PatchAlignment.FILL_OVERLAP, 1d);
...
//		Region region = new CaliforniaRegions.RELM_SOCAL();
		...
		for (FaultSection sect : matchingSects) {
			double cff = sumAgg.calc(sources, sect);
			System.out.println(sect.getName()+" (rake="+(float)sect.getAveRake()+"):\t"+(float)cff);
		}
