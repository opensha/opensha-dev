...
public class WrapperHazardCalc {
	...
	public static void main(String[] args) throws IOException {
		...
		NshmErf erf = new NshmErf(erfPath);
...
//		File outputDir = new File("/home/kevin/OpenSHA/UCERF4/batch_inversions/2022_09_20-nshm18-hazard-ask2014/results");
		
		Region modelReg = new CaliforniaRegions.RELM_TESTING();
		GriddedRegion gridReg = new GriddedRegion(modelReg, 0.2d, GriddedRegion.ANCHOR_0_0);
		File outputDir = new File("/home/kevin/OpenSHA/UCERF4/batch_inversions/2022_09_21-nshm18-ca-hazard-ask2014/results");
		...
		DiscretizedFunc[] linearXVals = {
				imtInfo.getDefaultHazardCurve(PGA_Param.NAME), imtInfo.getDefaultHazardCurve(SA_Param.NAME) };
...
//			System.out.println("Log curve for p="+periods[i]+"\n"+logXVals[i]);
		...
		for (int t=0; t<threads; t++) {
			CalcThread thread = new CalcThread(erf, gmpeRef, gridReg, periods, logXVals, linearXVals, curves, deque);
			thread.start();
		...
		System.out.println("Took "+(float)mins+" minutes to calculate "+gridReg.getNodeCount()+" sites");
		System.out.println("Rate: "+(mins*60d/gridReg.getNodeCount())+" curves per second");
		
		List<String> zipFileNames = new ArrayList<>();
		
		for (int p=0; p<periods.length; p++) {
			File curvesFile = new File(outputDir, SolHazardMapCalc.getCSV_FileName("curves", periods[p])+".gz");
...
//					System.out.println("Curve for "+loc+":\n"+logCurve);
				...
				System.out.println("DONE site "+index+"/"+nodeCount+" in "+(float)secs+" s"
						+ "\t("+(float)(100d*done/(double)nodeCount)+" % completed or running)");
			}
		}
