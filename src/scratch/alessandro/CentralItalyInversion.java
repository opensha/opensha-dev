...
import scratch.alessandro.logicTreeEnums.CentralItalySlipRatesEnum;

/**
 * This class reads Central Italy inversion data from files, provides methods for getting the various constraints, and runs the inversion.
 * 
 ...
 *
 */
public class CentralItalyInversion {
	...
	final static String PALEO_RATE_FILENAME = "paleorate.txt";
	...
	public CentralItalyInversion(CentralItalySlipRatesEnum centralItalySlipRatesEnum) {
		this.centralItalySlipRatesEnum = centralItalySlipRatesEnum;
		readData();
...
//				System.out.println(i+"\t"+split[0]+"\t"+rates[i]);
	...
	private void readData() {
		...
		double[] sectSlipRate = centralItalySlipRatesEnum.getSlipRateArray();
		double[] sectSlipRateStdDev = centralItalySlipRatesEnum.getSlipRateStdomArray();
...
//			System.out.println(param.getName()+"\t"+param.getValue());
	...
	public void makeHazardMaps(FaultSystemRuptureRateInversion fltSysRupInversion, double saPeriod, String dirName, boolean popupWindow) {
		...
		// make the site object and set values
		Site site = new Site(new Location(42.35, 13.39));	// Location will get over written
		for (Parameter<?> param : imr.getSiteParams()) {
			site.addParameter(param);
			System.out.println(param.getName()+"\t"+param.getValue());
		}
...
//				CPT cpt = GMT_CPT_Files.MAX_SPECTRUM.instance().rescale((0), (0.7));
...
//				map.setGenerateKML(true); // this tells it to generate a KML file that can be loaded into Google Earthe
	...
	public static void main(String []args) {
		...
		String dirName = ROOT_PATH+"OutputDataAndFigs_CentralItaly_test2";
	
		// Inversion name
		String name = "Central Italy Inversion";
		
		// Slip Rate
		CentralItalySlipRatesEnum slipRates = CentralItalySlipRatesEnum.NON_UNIFORM_MEAN;
...
//		long randomSeed = 1525892588112l; // for reproducibility; note that the last character here is the letter "l" to indicated a long value
		...
		Location hazCurveLoc = new Location(42.35, 13.39);	// L'Aquila
	    String hazCurveLocName = "L Aquila";
	    
	    // to make hazard maps
	    boolean makeHazardMaps = true;
		double saPeriodForHaz = 0.0;	// set as 0.0 for PGA

		// THIS IS THE END OF THE INVERSION SETTINGS

		// Create an instance of this inversion class
		CentralItalyInversion CentralItalyInversion = new CentralItalyInversion(slipRates);
				

		ArrayList<FaultSectionPrefData> fltSectDataList = CentralItalyInversion.getFaultSectionDataList();
		ArrayList<SegRateConstraint> sectionRateConstraints = CentralItalyInversion.getSectionRateConstraints();
		int[][] rupSectionMatrix = CentralItalyInversion.getRupSectionMatrix();
...
//		System.exit(0);
	    ...
	    switch (solutionType) {
	    		...
	    		case FROM_FILE:
			    	if(numSolutions==1) {
			    		...
			    		double[] rupRatesArray = CentralItalyInversion.readRuptureRatesFromFile(rupRatesFileName);
		    			...
		    			for(int i=0;i<numSolutions;i++) {
			    			...
			    			rupRatesArrayList.add(CentralItalyInversion.readRuptureRatesFromFile(rupRatesFileName));
	    ...
	    // hazard curve:
		if(hazCurveLoc != null) {
			CentralItalyInversion.writeAndOrPlotHazardCurve(fltSysRupInversion, hazCurveLoc, saPeriodForHaz, dirName, popUpPlots, hazCurveLocName);
		}
	    
	    // second parameter here is SA period; set as 0.0 for PGA:
		if(makeHazardMaps)
			CentralItalyInversion.makeHazardMaps(fltSysRupInversion, saPeriodForHaz, dirName, popUpPlots);
	}
}
