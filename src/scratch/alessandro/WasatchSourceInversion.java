 ...
 *
 */
public class WasatchSourceInversion {
...
//				sumOfSegNuclMF_NoParkfield.addIncrementalMagFreqDist(segNuclMFD); 
		...
		for(int m=0; m<sumOfSegPartMFDs.size();m++) aveOfSegPartMFDs.add(m, sumOfSegPartMFDs.getY(m)/num_seg);
...
//		mfd_graph.setY_AxisRange(1e-5, 1);
	...
	public static void main(String []args) {
		...
		// create instance of this java class:
		WasatchSourceInversion wasachSrcInv = new  WasatchSourceInversion();
...
//		double grConstraintRateScaleFactor = 0.83;  // for case where b=0
		...
		wasachSrcInv.doInversion(maxSubsectionLength,numSegForSmallestRups,deformationModel,
		...
		wasachSrcInv.writeFinalStuff();
		wasachSrcInv.writePredErrorInfo();
//		soSAF_SubSections.plotHistograms();

		
		String dirName = "Fig2_Data";
	    File file = new File(wasachSrcInv.ROOT_PATH+dirName);
	    file.mkdirs();
	    wasachSrcInv.plotStuff(dirName);
	    wasachSrcInv.plotOrWriteSegPartMFDs(dirName, true);
	    wasachSrcInv.writeAndPlotNonZeroRateRups(dirName, true);
	    wasachSrcInv.writeAndPlotRupRatesByEndPoints(dirName, true);
	    wasachSrcInv.writeAndPlotSubSectionsInGMT(dirName);
